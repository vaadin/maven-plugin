...
public class UpdateWidgetsetMojo extends AbstractGwtShellMojo {
    ...
    private static final String APP_WIDGETSET_FILE = APP_WIDGETSET_MODULE
    ...
    public final void doExecute()
        ...
        if ("cdn".equals(widgetsetMode) || "fetch".equals(widgetsetMode)) {
            ...
            File outputFile = new File(generatedSourceDirectory,
                    WSCDN_WIDGETSET_CLASS_NAME + ".java");

            try {
                triggerCdnBuild(wsReq, outputFile,
                        "fetch".equals(widgetsetMode));
            } catch (IOException e) {
    ...
    private final void updateLocalWidgetset()
            throws MojoExecutionException, MojoFailureException {

        File appwsFile = new File(generatedWidgetsetDirectory,
                APP_WIDGETSET_FILE);

        // compile one widgetset at a time
        String[] modules = getModules();
        if (modules.length == 1 && APP_WIDGETSET_MODULE.equals(modules[0])
                && appwsFile.exists()) {
            ...
            try {
                String template = IOUtil.toString(
                        getClass().getResourceAsStream("/AppWidgetset.tmpl"));
                if (!appwsFile.exists()) {
                ...
                try (FileInputStream fis = new FileInputStream(appwsFile)) {
                    ...
                    fis.close();
                    if (template.replaceAll("\\s", "")
                            .equals(generatedws.replaceAll("\\s", ""))) {
                        if (!appwsFile.delete()) {
                            getLogger()
                ...
                throw new MojoExecutionException(
                        "Failed to create AppWidgetset", e);
            }
     ...
     */
    protected void triggerCdnBuild(WidgetSetRequest wsReq, File outputFile,
            boolean fetch) throws IOException, MojoExecutionException {
        String wsName = null;
        String wsUrl = null;

        Connection conn = new Connection();
        WidgetSetResponse wsRes = conn.queryRemoteWidgetSet(wsReq, true);
        if (wsRes != null && (wsRes.getStatus() == PublishState.AVAILABLE // Compiled
                                                                          // and
                                                                          // published
                                                                 ...
                                                                 // widgetset)
        {
            ...
            throw new MojoExecutionException(
                    "Remote widgetset compilation failed: " + (wsRes != null
                            ? wsRes.getStatus() : " (no response)"));
        }

        PublishState status = fetch ? PublishState.AVAILABLE
                : wsRes.getStatus();
    ...
    private void createWidgetsetInfoClass(WidgetSetRequest wsReq,
            ...
            throws IOException {
        String widgetsetInfo = IOUtil.toString(
                getClass().getResourceAsStream("/widgetsetinfo.tmpl"));
        ...
        if (wsName != null && wsUrl != null) {
            getLog().info("Widgetset config created to "
                    + outputFile.getAbsolutePath() + ". Public URL: " + wsUrl);
        } else {
            getLog().info("Widget set created to "
                    + outputFile.getAbsolutePath() + ".");
        }
    }

    private void updateWidgetset(String module, boolean generated)
            throws MojoExecutionException {
        ...
        if (null != resources) {
            for (Object resObj : resources) {
                ...
                if (resourceDirectory.exists()) {
                    ...
                    getLog().warn("Ignoring missing resource directory: "
                            + resourceDirectory);
                }
            }
        }
    ...
    protected File setupGeneratedWidgetsetDirectory() {
        if (!generatedWidgetsetDirectory.exists()) {
            getLog().debug("Creating target directory "
                    + generatedWidgetsetDirectory.getAbsolutePath());
            generatedWidgetsetDirectory.mkdirs();
        }

        getLog().debug("Add resource directory "
                + generatedWidgetsetDirectory.getAbsolutePath());
