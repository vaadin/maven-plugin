...
public class EclipseMojo extends AbstractGwtModuleMojo {
    ...
    protected void setupExplodedWar() throws MojoExecutionException {
        try {
            ...
            throw new MojoExecutionException(
                    "Failed to create Jetty exploded webapp", ioe);
        }
    }
    ...
    private void createLaunchConfigurationForHostedModeBrowser(String module)
            throws MojoExecutionException {
        try {
            File launchFile = new File(getProject().getBasedir(),
                    readModule(module).getPath() + ".launch");
            if (launchFile.exists()) {
                getLog().info("launch file exists " + launchFile.getName()
                        + " skip generation ");
                return;
            }
            ...
            context.put("out", getOutputDirectory().getAbsolutePath()
                    .substring(basedir + 1));
            context.put("war",
                    hostedWebapp.getAbsolutePath().substring(basedir + 1));
            ...
            String templateName = useGoogleEclipsePlugin ? "google.fm"
                    : "launch.fm";
            Template template = cfg.getTemplate(templateName, "UTF-8");
            template.process(context, configWriter);
            configWriter.flush();
            configWriter.close();
            getLog().info("Write launch configuration for GWT module : "
