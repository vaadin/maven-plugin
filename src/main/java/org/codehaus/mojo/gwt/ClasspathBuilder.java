...
public class ClasspathBuilder extends AbstractLogEnabled {
     ...
     */
    public Collection<File> buildClasspathList(final MavenProject project,
            final String scope, Set<Artifact> artifacts, boolean isGenerator)
            throws ClasspathBuilderException {
        getLogger()
                .debug("establishing classpath list (scope = " + scope + ")");
        ...
        // GWT compiler that should not be accessible in
        ...
        if (scope.equals(SCOPE_TEST)) {
            ...
            for (Artifact artifact : artifacts) {
                ...
                if (SCOPE_COMPILE.equals(artifactScope)
                        || SCOPE_PROVIDED.equals(artifactScope)
     ...
     * @param scope
     */
    public void addSourcesWithActiveProjects(final MavenProject project,
            final Collection<File> items, final String scope) {
        ...
        for (Artifact artifact : scopeArtifacts) {
            String projectReferenceId = getProjectReferenceId(
                    ...
                    artifact.getVersion());
            MavenProject refProject = project.getProjectReferences()
                    .get(projectReferenceId);
            if (refProject != null
     ...
     * @param scope
     */
    public void addResourcesWithActiveProjects(final MavenProject project,
            final Collection<File> items, final String scope) {
        final List<Artifact> scopeArtifacts = getScopeArtifacts(project, scope);
        ...
        for (Artifact artifact : scopeArtifacts) {
            String projectReferenceId = getProjectReferenceId(
                    artifact.getGroupId(), artifact.getArtifactId(),
                    artifact.getVersion());
            MavenProject refProject = (MavenProject) project
                    .getProjectReferences().get(projectReferenceId);
            if (refProject != null) {
     ...
     * @return
     */
    private List<Artifact> getScopeArtifacts(final MavenProject project,
            final String scope) {
     ...
     * @return
     */
    private List<String> getSourceRoots(final MavenProject project,
            final String scope) {
     ...
     * @return
     */
    private List<Resource> getResources(final MavenProject project,
            final String scope) {
        if (SCOPE_COMPILE.equals(scope) || SCOPE_RUNTIME.equals(scope)) {
     ...
     * @param sourceRoots
     */
    private void addSources(final Collection<File> items,
            final Collection<String> sourceRoots) {
     ...
     * @param resources
     */
    private void addResources(final Collection<File> items,
     ...
     * @return
     */
    private String getProjectReferenceId(final String groupId,
