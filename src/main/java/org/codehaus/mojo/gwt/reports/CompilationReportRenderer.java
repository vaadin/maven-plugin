...
public class CompilationReportRenderer extends AbstractMavenReportRenderer {
    ...
    public CompilationReportRenderer(final Sink sink,
            final List<GwtModule> gwtModules, Log log, boolean reportsAvailable,
            String compilerReportsPath, boolean compilerReport, I18N i18n,
            Locale locale) {
        super(sink);
    ...
    protected void renderBody() {
        ...
        if (!this.reportsAvailable) {
            ...
            if (compilerReport) {
                sink.text(
                        getI18nString(locale, "compiler.report.none.warning"));
            } else {
                sink.text(
                        getI18nString(locale, "compiler.report.soyc.warning"));
            }
            sink.bold_();
            sink.paragraph_();
        } else {
            ...
            for (GwtModule gwtModule : this.gwtModules) {
                ...
                if (StringUtils.isNotBlank(compilerReportsPath)) {
                    sink.link("./" + compilerReportsPath + "/"
                            + gwtModule.getPath() + "/index.html");
                } else {
