...
public class ServletAnnotationFinder extends AbstractLogEnabled {
    ...
    private MetadataReader getMetadataReader(String className,
        ...
        Resource resource = resourceResolver
                .getResource(resourcePath + ".class");
        if (resource.exists()) {
    ...
    private boolean extendsRemoteServlet(ClassMetadata classMetadata,
        ...
        if (classMetadata.hasSuperClass()) {
            ...
            MetadataReader r = getMetadataReader(
                    classMetadata.getSuperClassName(), factory,
                    resourceResolver);
            return extendsRemoteServlet(r.getClassMetadata(), factory,
                    resourceResolver);
        }
        return false;
    }
    ...
    private AnnotationMetadata getAnnotationMetadataIfServlet(
            MetadataReader metadataReader, MetadataReaderFactory factory,
            PathMatchingResourcePatternResolver resourceResolver) {
        ClassMetadata classMetadata = metadataReader.getClassMetadata();

        try {
            if (classMetadata.isConcrete() && extendsRemoteServlet(
                    classMetadata, factory, resourceResolver)) {
                for (String i : metadataReader.getClassMetadata()
                        .getInterfaceNames()) {
                    MetadataReader r = getMetadataReader(i, factory,
                            resourceResolver);
                    if (r != null && r.getAnnotationMetadata().hasAnnotation(
                            REMOTE_SERVICE_RELATIVE_PATH_ANNOTATION_NAME)) {
    ...
    public Set<ServletDescriptor> findServlets(String packageName,
            String startPath, ClassLoader classLoader) throws IOException {
        ...
        Resource[] resources = pathMatchingResourcePatternResolver
                .getResources(patternFinder);
        SimpleMetadataReaderFactory simpleMetadataReaderFactory = new SimpleMetadataReaderFactory();
        getLogger().debug("springresource " + resources.length + " for pattern "
                + patternFinder);
