...
public class GwtWebInfProcessor {
    private static final String[] BEFORE_SERVLETS = { "icon", "display-name",
            "description", "distributable", "context-param", "filter",
            "filter-mapping", "listener", "servlet" };

    private static final String[] AFTER_SERVLETS = { "servlet-mapping",
    ...
    private static final String[] BEFORE_MAPPINGS = { "icon", "display-name",
            "description", "distributable", "context-param", "filter",
            "filter-mapping", "listener", "servlet", "servlet-mapping" };

    private static final String[] AFTER_MAPPINGS = { "session-config",
    ...
    public void process(File webXml,
            Collection<ServletDescriptor> servletDescriptors) throws Exception {
        process(webXml, webXml, servletDescriptors);
    }

    public void process(File sourceWebXml, File mergeWebXml,
            Collection<ServletDescriptor> servletDescriptors) throws Exception {
    ...
    private Document insertServlets(File webXml,
        ...
        int insertAfter = getInsertPosition(webapp, BEFORE_SERVLETS,
                AFTER_SERVLETS);
        for (Iterator<ServletDescriptor> it = servletDescriptors.iterator(); it
                .hasNext();) {
            ServletDescriptor d = it.next();
            XPath path = XPath
                    .newInstance("/web-app/servlet/servlet-name[text() = '"
                            + d.getName() + "']");
        ...
        insertAfter = getInsertPosition(webapp, BEFORE_MAPPINGS,
                AFTER_MAPPINGS);
    ...
    private int getInsertPosition(Element webapp, String[] startAfter,
            String[] stopBefore) throws JDOMException, IOException {
