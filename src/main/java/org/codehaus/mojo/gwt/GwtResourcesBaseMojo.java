...
abstract class GwtResourcesBaseMojo extends AbstractGwtModuleMojo {
    ...
    protected Collection<ResourceFile> getAllResourceFiles()
        ...
        try {
            ...
            sourcesAndResourcesPath
                    .addAll(getProject().getCompileSourceRoots());
            ...
            for (String name : getModules()) {
                ...
                sourcesAndResources
                        .add(getDescriptor(module, sourcesAndResourcesPath));
                int count = 1;

                for (String source : module.getSources()) {
                    getLog().debug("GWT sources from " + name + '.' + source);
                    Collection<ResourceFile> files = getAsResources(module,
                            source, sourcesAndResourcesPath, "**/*.java");
                    sourcesAndResources.addAll(files);
                    count += files.size();

                    Collection<ResourceFile> uifiles = getAsResources(module,
                            source, sourcesAndResourcesPath, "**/*.ui.xml");
                    sourcesAndResources.addAll(uifiles);
                    count += uifiles.size();
                }
                for (String source : module.getSuperSources()) {
                    getLog().debug(
                            "GWT super-sources from " + name + '.' + source);
                    Collection<ResourceFile> files = getAsResources(module,
                            source, sourcesAndResourcesPath, "**/*.java");
                    sourcesAndResources.addAll(files);
                    count += files.size();
     ...
     * @param name
     */
    private Collection<ResourceFile> getAsResources(GwtModule module,
            String source, Set<String> paths, String include) {
        ...
        for (String path : paths) {
            ...
            scanner.setIncludes(
                    new String[] { pattern + '/' + source + '/' + include });
            scanner.scan();
    ...
    private ResourceFile getDescriptor(GwtModule module, Set<String> paths)
        ...
        throw new MojoExecutionException(
                "Failed to retrieve GWT descriptor in project sources "
                        + moduleFilePath);
    }
